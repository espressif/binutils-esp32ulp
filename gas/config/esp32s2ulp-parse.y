/* esp32s2ulp-parse.y  Espressif ESP32S2ULP Parser
...
#include "esp32s2ulp-aux.h"  /* Opcode generating auxiliaries.  */
...
#define ALU_ADDR(dst, src1, src2)  \
	esp32s2ulp_gen_alu_ADDR(dst.regno, src1.regno, src2.regno)

#define ALU_ADDI(dst, src1, imm)  \
	esp32s2ulp_gen_alu_ADDI(dst.regno, src1.regno, imm)

#define JUMP_R(dst, cond)  \
	esp32s2ulp_gen_jump_r(dst.regno, cond)
#define JUMP_I(dst, cond)  \
	esp32s2ulp_gen_jump_i(dst, cond)

#define CMD_JUMPR(step, tresh, cond)  \
	esp32s2ulp_cmd_jump_relr(step, tresh, cond)
#define CMD_JUMPS(step, tresh, cond)  \
	esp32s2ulp_cmd_jump_rels(step, tresh, cond)

#define MOVE_C2R(dst, data)  \
	esp32s2ulp_move_const2reg(dst.regno, data)
#define MOVE_E2R(dst, addr)  \
	esp32s2ulp_move_addr2reg(dst.regno, addr)
...
#define WR_MEM_STLR(lh, dst, src, offset)  \
	esp32s2ulp_wr_mem_stlh(lh, dst.regno,  src.regno, offset)

#define WR_MEM_ST32(dst, src, offset, label) \
	esp32s2ulp_wr_mem_st32(dst.regno,  src.regno, offset, label)

#define WR_MEM_STI32(dst, src, label) \
	esp32s2ulp_wr_mem_sti32(dst.regno,  src.regno, label)

#define WR_MEM_ST_L(lh, dst, src, offset, label)  \
	esp32s2ulp_wr_mem_st_l(lh, dst.regno,  src.regno, offset, label)

#define WR_MEM_STI_L(dst, src, label)  \
	esp32s2ulp_wr_mem_sti_l(dst.regno,  src.regno, label)

#define WR_MEM_STI(dst, src)  \
	esp32s2ulp_wr_mem_sti(dst.regno,  src.regno)


#define WR_MEM_STO(offset)  \
	esp32s2ulp_wr_mem_offset(offset)
// ----------------------------------------------

#define RD_MEM_ADDR(lh, dst, src, addr)  \
	esp32s2ulp_rd_mem_addr(lh, dst.regno,  src.regno, addr)
#define RD_MEM_OFFSET(dst, src, addr)  \
	esp32s2ulp_rd_mem_offset(dst.regno,  src.regno, addr)

#define CMD_REG_RD(addr, high, low) \
	esp32s2ulp_cmd_reg_rd(addr, high, low)
#define CMD_REG_WR(addr, high, low, data) \
	esp32s2ulp_cmd_reg_wr(addr, high, low, data)

#define CMD_I2C_RD(i2c_addr, high, low, i2c_sel) \
	esp32s2ulp_cmd_i2c_rd(i2c_addr, high, low, i2c_sel)
#define CMD_I2C_WR(i2c_addr, high, low, i2c_sel, data) \
	esp32s2ulp_cmd_i2c_wr(i2c_addr, high, low, i2c_sel, data)
...
#define CMD_WAKEUP(wake)  \
	 esp32s2ulp_cmd_wakeup(wake)
#define CMD_SLEEP(cycles)  \
	 esp32s2ulp_cmd_sleep(cycles)
#define CMD_WAIT(cycles)  \
	 esp32s2ulp_cmd_wait(cycles)

#define CMD_TSENS(dreg, delay)  \
	esp32s2ulp_cmd_tsens(dreg.regno, delay)

#define CMD_ADC(dreg, sar_sel, mux)  \
	esp32s2ulp_cmd_adc(dreg.regno, sar_sel, mux)

#define CMD_ALUR(dst, src1, src2, operation)  \
	 esp32s2ulp_gen_alu_r(dst.regno,src1.regno,src2.regno, operation)
#define CMD_ALUI(dst, src1, imm, operation)  \
	 esp32s2ulp_gen_alu_i(dst.regno, src1.regno, imm, operation)
#define CMD_ALUI_DIR(dst, imm, operation)  \
	 esp32s2ulp_gen_alu_i(dst.regno, 0, imm, operation)

#define CMD_STAGE(dir, imm)  \
		esp32s2ulp_cmd_stage(dir, imm)
#define CMD_STAGE_RST()  \
		esp32s2ulp_cmd_stage_rst()
